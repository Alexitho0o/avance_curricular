...
def _resolve_for_ing_act(series: pd.Series | None, index: pd.Index) -> pd.Series:
    """Resuelve FOR_ING_ACT con fallback operativo.

    Regla: si la fuente trae valor numérico válido (1..11), se conserva.
    Si no existe o es inválido, se fija en 10 (otras formas de ingreso).
    """
    if series is None:
        return pd.Series(10, index=index, dtype="Int64")
    vals = pd.to_numeric(series, errors="coerce")
    vals = vals.where(vals.between(1, 11), pd.NA).fillna(10)
...
def construir_matricula_unificada_control(mat_ac: pd.DataFrame, df_equiv: pd.DataFrame) -> pd.DataFrame:
    ...
    col_for_ing_act = _pick_first_column(
        mat_ac, ["FOR_ING_ACT", "FORMA_INGRESO", "FORMAINGRESO", "FORMA_INGRESO_ACTUAL", "TIPO_INGRESO"]
    )
    out["FOR_ING_ACT"] = _resolve_for_ing_act(mat_ac[col_for_ing_act] if col_for_ing_act else None, out.index)
...
def ejecutar_pipeline_matricula_unificada_legacy_like(
    ...
    col_for_ing_act = _pick_first_column(
        src, ["FOR_ING_ACT", "FORMA_INGRESO", "FORMAINGRESO", "FORMA_INGRESO_ACTUAL", "TIPO_INGRESO"]
    )
    ...
    out["VERSION"] = pd.NA
    out["FOR_ING_ACT"] = _resolve_for_ing_act(src[col_for_ing_act] if col_for_ing_act else None, out.index)
    if col_anio_ing:
