...
def main() -> int:
    ...
    # PLAN_NORM en este punto corresponde exclusivamente al plan
    # observado en Hoja1. Se renombra para evitar colisión posterior
    # con PLAN_NORM del resumen canónico de mallas.
    if "PLAN_NORM" in p5809_control.columns:
        p5809_control = p5809_control.rename(
            columns={
                "PLAN_NORM": "PLAN_HOJA1_NORM",
            }
        )
    ...
    # Control defensivo: antes de incorporar el resumen canónico,
    # no debe existir otra columna genérica PLAN_NORM.
    if "PLAN_NORM" in p5809_control.columns:
        raise RuntimeError(
            "Colisión previa inesperada: PLAN_NORM ya existe "
            "antes del cruce con resumen_plan."
        )
    ...
    if "PLAN_NORM" not in p5809_control.columns:
        raise RuntimeError(
            "El cruce con resumen_plan no generó PLAN_NORM."
        )
