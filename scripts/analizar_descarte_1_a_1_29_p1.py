...
def evaluar_caso(fila, excluir=None):
    ...
    # El documento se conserva como alerta. No funciona como
    # veto automático, porque estos casos ya fueron identificados
    # previamente mediante identidad personal exacta.
    if conflicto_documento:
        observaciones.append(
            "Alerta: el documento observado no coincide con el "
            "documento de 5809. Requiere revisión, pero no oculta "
            "los conflictos de carrera, trayectoria o nivel."
        )

    if nivel_fuera_rango:
        ...
        observaciones.append(
            "Existen múltiples trayectorias, carreras "
            "o niveles posibles."
        )
        ...
        if conflicto_documento:
            decision = (
                "CANDIDATO_CON_ALERTA_DOCUMENTO_"
                "REQUIERE_VALIDACION"
            )
            observaciones.append(
                "Carrera y nivel son compatibles, pero debe "
                "confirmarse la diferencia documental."
            )
        else:
