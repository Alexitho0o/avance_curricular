...
if len(bloqueados) != 42:
    raise RuntimeError(
        f"Se esperaban 42 planes bloqueados y se encontraron "
        f"{len(bloqueados)}."
    )
...
if len(resultado) != 42:
    raise RuntimeError(
        "La consolidación no conserva los 42 planes."
    )
...
resultado.to_csv(
    RESULTADOS
    / "01_DIAGNOSTICO_42_PERIODIZACIONES.tsv",
    sep="\t",
    index=False,
)
...
print()
print("=" * 108)
print("EXPEDIENTE DE LOS 42 PLANES SIN PERIODIZACIÓN DEMOSTRADA")
print("=" * 108)
