...
if casos["RUT_CRUCE"].eq("").any():
    faltantes = casos[
        casos["RUT_CRUCE"].eq("")
    ]["ID_FILA_5809"].tolist()

    raise RuntimeError(
        "Existen casos sin RUT para cruzar: "
        + " | ".join(faltantes)
    )
...
hoja_preferida = next(
    (
        hoja
        for hoja in libro_promedios.sheet_names
        if norm_col(hoja) == "MATRIZ"
    ),
    libro_promedios.sheet_names[0],
)

promedios = pd.read_excel(
    PROMEDIOS,
...
col_rut_promedios = buscar_columna(
    promedios,
    [
        "RUT",
        "RUT_ALUMNO",
...
col_codcli_promedios = buscar_columna(
    promedios,
    [
        "CODCLI",
        "CODIGO_ALUMNO",
...
validaciones = pd.DataFrame([
    ...
    {
        "VALIDACION": "RUT_CASOS_COMPLETOS",
        "RESULTADO": (
            "OK"
            if resultado[
                "RUT_CRUCE"
