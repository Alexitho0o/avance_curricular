...
def valor_numero_salida(valor: Any) -> str:
    """
    Convierte un resultado numérico a texto compatible con las
    columnas oficiales de precarga.
...
def main() -> int:
    ...
    # Las columnas oficiales provienen de un CSV leído como texto.
    # Se conservan como texto y los cálculos se formatean antes de
    # asignarlos. Esto evita escribir valores como 29.0 y elimina
    # la incompatibilidad entre dtype string y float.
    columnas_creditos_salida = [
        "TOTAL_UNIDADES_MEDIDA",
        *COLUMNAS_ANUALES.values(),
    ]

    for columna in columnas_creditos_salida:
        candidato_5810[columna] = (
            candidato_5810[columna]
            .astype("object")
        )
    ...
    if not resumen_indexado.index.is_unique:
        duplicados_indice = (
            resumen_indexado.index[
                resumen_indexado.index.duplicated(
                    keep=False
                )
            ]
            .unique()
        ...
        raise RuntimeError(
            "resumen_plan contiene CODPESTUD duplicados: "
            + " | ".join(
                str(valor)
                for valor in duplicados_indice[:20]
            )
        )

    for _, fila in p5810.iterrows():
        codpestud = norm_codigo(
            fila["CODPESTUD_RESUELTO"]
        )
    ...
    # Control físico antes de eliminar el identificador de auditoría.
    if len(candidato_5810) != len(p5810):
        raise RuntimeError(
            "El candidato 5810 cambió la cantidad de filas."
        )
    ...
    # Validar que las columnas calculadas no contengan la forma
    # textual de decimales enteros, por ejemplo 29.0.
    valores_decimal_entero = []
    ...
    for columna in columnas_creditos_salida:
        ...
        mask_decimal_entero = serie.str.match(
            r"^-?\\d+\\.0+$",
            na=False,
        )

        if mask_decimal_entero.any():
            valores_decimal_entero.append({
                "COLUMNA": columna,
                "CASOS": int(
                    mask_decimal_entero.sum()
                ),
            })

    if valores_decimal_entero:
        detalle = " | ".join(
            f"{item['COLUMNA']}={item['CASOS']}"
            for item in valores_decimal_entero
        )

        raise RuntimeError(
            "Persisten números enteros escritos con .0 "
            "en candidato 5810: "
            + detalle
        )
