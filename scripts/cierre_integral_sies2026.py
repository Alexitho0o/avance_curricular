...
def make_resume(exec_dir: Path, phase: int, reason: str = "") -> None:
    command = f'python3 scripts/cierre_integral_sies2026.py --execution-dir "{exec_dir}" --desde-fase {phase}'
    content = [
        "# Reanudar desde aqui",
        "",
...
def official_rules() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    rules.insert(1, "PROCESO", "Avance Curricular SIES 2026")
...
def phase4(exec_dir: Path, state: dict[str, Any]) -> bool:
    mark_phase(exec_dir, state, 4, "EN_EJECUCION", next_phase=4)
    paths = control_paths(exec_dir)
    out = exec_dir / "03_CONCILIACION_CARRERAS"
    final43 = (
        PROCESO_DIR
        ...
        / "CIERRE_FINAL_43_RESUELTAS_0_PENDIENTES_20260701_171409"
        / "02_RESULTADOS"
        / "01_CONCILIACION_FINAL_43.tsv"
    )
    desbloqueo = (
        PROCESO_DIR
        / "04_gobernanza_mallas"
        / "03_auditorias"
        / "DESBLOQUEO_NIVELES_EVIDENCIAS_CARRERAS_43_20260701_180302"
    )
    matriz_desbloqueo = desbloqueo / "05_RESULTADOS" / "matriz_funcional_actualizada_43.tsv"
    distribucion = desbloqueo / "04_RECALCULO" / "distribucion_anual_recalculada.tsv"
    resumen_desbloqueo = desbloqueo / "06_REPORTES" / "RESUMEN_DESBLOQUEO_FUNCIONAL_CARRERAS_43.md"

    required = [final43, matriz_desbloqueo, distribucion, resumen_desbloqueo]
        ...
        for p in missing:
            append_tsv(
                paths["bloqueos"],
                {
                    "FECHA": now_iso(),
                    "FASE": "FASE_4",
                    "MOTIVO": "FALTA_ARTEFACTO_PREVIO_RELEVANTE",
                    "DETALLE": str(p),
                    "ACCION": "Recrear o ubicar artefacto de conciliacion de carreras antes de continuar.",
                },
                ["FECHA", "FASE", "MOTIVO", "DETALLE", "ACCION"],
            )
        mark_phase(exec_dir, state, 4, "BLOQUEADA", next_phase=4, bloqueos=len(missing))
        make_resume(exec_dir, 4, "Faltan artefactos para conciliacion carreras.")
        return False

    conciliacion = pd.read_csv(final43, sep="\t", dtype=str, keep_default_na=False)
    matriz = pd.read_csv(matriz_desbloqueo, sep="\t", dtype=str, keep_default_na=False)
    dist = pd.read_csv(distribucion, sep="\t", dtype=str, keep_default_na=False)
    ...
    mapeo = conciliacion[
        [
            ...
            "CODCARR_CANONICO_CIERRE",
            "ESTADO_CIERRE",
            "ORIGEN_ESTADO_CIERRE",
            "NIVEL_RESPALDO_DECISION",
            "FUNDAMENTO",
        ]
    ].copy()
    mapeo = mapeo.rename(
        columns={
            "PLAN_DE_ESTUDIO_CANONICO_CIERRE": "CODPESTUD_PLAN_INSTITUCIONAL",
            "CODCARR_CANONICO_CIERRE": "CODCARR_PLAN_INSTITUCIONAL",
        }
    )
    planes_multiples = (
        mapeo.groupby("CODIGO_UNICO", dropna=False)["PLAN_ESTUDIOS"]
        .nunique()
        .reset_index(name="PLANES_DISTINTOS")
    )
    planes_multiples = planes_multiples[planes_multiples["PLANES_DISTINTOS"].gt(1)]
    planes_no_resueltos = mapeo[
        mapeo["CODPESTUD_PLAN_INSTITUCIONAL"].astype(str).str.strip().eq("")
        | ~mapeo["ESTADO_CIERRE"].astype(str).str.contains("RESUELTO", na=False)
    ].copy()

    blocked = matriz[
        ~matriz.get("APTO_PARA_MATERIALIZAR", "").astype(str).eq("SI")
    ].copy()
    if not blocked.empty:
        blocked["FUENTE_FALTANTE"] = "Confirmacion institucional/documental de semantica NIVEL y periodizacion anual por plan."
        blocked["MENSAJE"] = (
            "Las fuentes disponibles en este proyecto no entregan informacion suficiente "
            "para resolver este caso con certeza. Falta: confirmacion institucional/documental "
            "de la semantica de NIVEL y su distribucion anual."
        )
    ...
    mapeo.to_csv(out / "MAPEO_CODIGO_UNICO_PLAN.tsv", sep="\t", index=False)
    planes_multiples.to_csv(out / "PLANES_MULTIPLES.tsv", sep="\t", index=False)
    planes_no_resueltos.to_csv(out / "PLANES_NO_RESUELTOS.tsv", sep="\t", index=False)
    dist_out.to_csv(out / "DISTRIBUCION_UNIDADES_POR_ANIO.tsv", sep="\t", index=False)
    blocked.to_csv(out / "PLANES_DISTRIBUCION_NO_DEMOSTRADA.tsv", sep="\t", index=False)

    resumen = pd.DataFrame(
        [
            ["CODIGO_UNICO en conciliacion 43/0", len(conciliacion)],
            ["Planes identidad resuelta", len(mapeo) - len(planes_no_resueltos)],
            ["Planes identidad no resuelta", len(planes_no_resueltos)],
            ["CODIGO_UNICO con mas de un PLAN_ESTUDIOS", len(planes_multiples)],
            ["Carreras aptas para materializar segun desbloqueo", int(matriz["APTO_PARA_MATERIALIZAR"].eq("SI").sum())],
            ["Carreras bloqueadas o con evidencia faltante", len(blocked)],
        ],
        columns=["INDICADOR", "VALOR"],
    )
    write_xlsx(
        out / "CONCILIACION_CARRERAS_PLANES.xlsx",
        {
            "RESUMEN": resumen,
            "MAPEO_CODIGO_PLAN": mapeo,
            "DISTRIBUCION_ANUAL": dist_out,
            "BLOQUEOS_DISTRIBUCION": blocked,
            "PLANES_MULTIPLES": planes_multiples,
            "PLANES_NO_RESUELTOS": planes_no_resueltos,
        },
    )
    ...
    append_tsv(
        ...
        {
            "FASE": "FASE_4",
            "VALIDACION": "IDENTIDAD_PLAN_CARRERA_43",
            "ESTADO": "OK",
            "DETALLE": "43 CODIGO_UNICO con CODPESTUD resuelto en cierre 43/0.",
        },
    ...
    append_tsv(
        ...
        {
            "FASE": "FASE_4",
            "VALIDACION": "DISTRIBUCION_ANUAL_DEMOSTRADA",
            "ESTADO": "BLOQUEA",
            "DETALLE": f"{len(blocked)} carreras no aptas para materializar por evidencia faltante.",
        },
        ["FASE", "VALIDACION", "ESTADO", "DETALLE"],
    )
    append_tsv(
        paths["bloqueos"],
        {
            "FECHA": now_iso(),
            "FASE": "FASE_4",
            "MOTIVO": "DISTRIBUCION_ANUAL_NO_DEMOSTRADA",
            "DETALLE": (
                f"{len(blocked)} carreras siguen sin confirmacion documental/institucional "
                "de la semantica de NIVEL; no corresponde generar Carreras candidato ni CSV."
            ),
            "ACCION": "Obtener fuente institucional de periodizacion/distribucion anual por plan o documentar resolucion funcional.",
        },
        ["FECHA", "FASE", "MOTIVO", "DETALLE", "ACCION"],
    )
    append_tsv(
        paths["decisiones"],
        {
            "FECHA": now_iso(),
            "FASE": "FASE_4",
            "DECISION": "NO_GENERAR_CANDIDATO_CARRERAS",
            "EVIDENCIA": str(resumen_desbloqueo),
            "IMPACTO": "Se bloquea avance a validacion de unidades y construccion de Carreras.",
        },
        ["FECHA", "FASE", "DECISION", "EVIDENCIA", "IMPACTO"],
    )
    ...
    save_json(
        paths["checkpoint"],
        {
            ...
            "estado": "BLOQUEADA",
            "fecha": now_iso(),
            "entradas": [str(final43), str(matriz_desbloqueo), str(distribucion), str(resumen_desbloqueo)],
            "salidas": [
                str(out / "CONCILIACION_CARRERAS_PLANES.xlsx"),
                str(out / "MAPEO_CODIGO_UNICO_PLAN.tsv"),
                str(out / "PLANES_DISTRIBUCION_NO_DEMOSTRADA.tsv"),
            ],
            "motivo": "Distribucion anual no demostrada para 42 carreras.",
        },
    )
    make_resume(exec_dir, 4, "Resolver evidencia faltante de distribucion anual antes de continuar.")
    mark_phase(exec_dir, state, 4, "BLOQUEADA", next_phase=4, bloqueos=1)
    return False
