...
# ============================================================
# 7A. EXTRAER CÓDIGO DE ALUMNO EMBEBIDO
# ============================================================

patron_codigo_alumno_modelo = re.compile(
    r"(?:CODIGO|CÓDIGO)?\s*ALUMNO\s*:\s*([A-Z0-9]+)",
    flags=re.IGNORECASE,
)

def extraer_codigo_alumno_modelo(valor):
    texto = texto_limpio(valor)

    if not texto:
        return ""

    coincidencia = patron_codigo_alumno_modelo.search(texto)
...
# ============================================================
# 7B. EXTRAER METADATOS DE LOS DOS CASOS MODELO
# ============================================================

def extraer_valor_rotulado(valor, rotulo):
    texto = texto_limpio(valor)

    patron = re.compile(
        rf"(?:CODIGO|CÓDIGO)?\s*{rotulo}\s*:\s*(.+)$",
        flags=re.IGNORECASE,
    )
...
for (
    archivo,
    hoja,
), grupo in celdas_avance.groupby(
    ...
    for valor in grupo["VALOR_ORIGINAL"]:
        if not codigo_alumno:
            codigo_alumno = extraer_codigo_alumno_modelo(
                valor
            )

        if not carrera:
            carrera = extraer_valor_rotulado(
                valor,
                "CARRERA",
            )

        if not plan:
            plan = extraer_valor_rotulado(
                valor,
                "PLAN(?:\\s+ESTUDIO)?",
            )
    ...
    for valor in grupo["VALOR_ORIGINAL"]:
        ...
        if coincidencia_periodo:
            periodos.append(
                int(coincidencia_periodo.group(1))
            )
    ...
    if codigo_alumno:
        ...
        if max_periodo == 20:
            tipo_periodizacion = (
                "ANTIGUA_EXTENDIDA_20_PERIODOS_4_ANIOS"
            )
            regla_observada = (
            ...
            tipo_periodizacion = (
                "ESTRUCTURA_NO_CLASIFICADA"
            )
            regla_observada = ""

        modelos_avance.append({
            "FUENTE_ARCHIVO": archivo,
            "FUENTE_HOJA": hoja,
            "CODCLI_MODELO": norm_codigo(
                codigo_alumno
            ),
            "CARRERA_MODELO": norm_codigo(
                carrera
            ),
            "PLAN_MODELO": norm_codigo(
                plan
            ),
            "PERIODO_MAX_MODELO": max_periodo,
...
# Los reportes no dejan el código en una celda aislada.
# El contenido observado es, por ejemplo:
# "Código Alumno: 20181IECIREOL015".
# Se extrae el código desde cualquier texto de la celda.

patron_codigo_alumno = re.compile(
    r"(?:CODIGO|CÓDIGO)?\s*ALUMNO\s*:\s*([A-Z0-9]+)",
    flags=re.IGNORECASE,
)

def extraer_codigo_alumno(valor):
    texto = texto_limpio(valor)

    if not texto:
        return ""

    coincidencia = patron_codigo_alumno.search(texto)
    ...
    # Control adicional: si un CODCLI objetivo está contenido
    # literalmente en la celda, también se registra.
    texto_norm = norm_codigo(texto)

    contenidos = [
        codigo
        for codigo in codcli_objetivo
...
celdas_avance["CODIGO_ALUMNO_EXTRAIDO"] = (
    celdas_avance["VALOR_ORIGINAL"]
    .map(extraer_codigo_alumno)
)

coincidencias_codcli = celdas_avance[
    celdas_avance[
        "CODIGO_ALUMNO_EXTRAIDO"
    ].isin(codcli_objetivo)
].copy()

coincidencias_codcli[
    "VALOR_CODIGO_NORM"
] = coincidencias_codcli[
    "CODIGO_ALUMNO_EXTRAIDO"
]
...
# ============================================================
# 10B. CLASIFICAR ESTRUCTURA HISTÓRICA
# ============================================================

resultado[
    "TIPO_PERIODIZACION_PROPUESTA"
] = ""

resultado[
    "REGLA_CONVERSION_PROPUESTA"
] = ""

resultado[
    "NIVEL_APTO_PARA_CONVERSION"
] = "NO"

resultado[
    "FUNDAMENTO_PERIODIZACION"
] = ""
...
for indice, fila in resultado.iterrows():
    ...
    # Prioridad 1: coincidencia exacta del CODCLI
    modelo_exacto = modelos_avance[
        modelos_avance[
            "CODCLI_MODELO"
        ].isin(codcli_lista)
    ]

    if len(modelo_exacto) == 1:
        modelo = modelo_exacto.iloc[0]
        ...
        resultado.at[
            ...
            "El CODCLI coincide exactamente con uno de los "
            "reportes institucionales de Avance de Malla."
        )

        continue

    # Prioridad 2: carrera como variable secundaria.
    # Solo clasifica el tipo; no aprueba el nivel todavía.
    modelos_carrera = modelos_avance[
        modelos_avance[
            "CARRERA_MODELO"
        ].isin(
            carreras_lista
    ...
    modelos_carrera = modelos_carrera[
        modelos_carrera[
            "CARRERA_MODELO"
        ].ne("")
    ]
    ...
    if len(tipos) == 1:
        ...
        resultado.at[
            indice,
            "NIVEL_APTO_PARA_CONVERSION",
        ] = "PENDIENTE_VALIDACION_CODCLI_PLAN"

        resultado.at[
            indice,
            "FUNDAMENTO_PERIODIZACION",
        ] = (
            "La carrera coincide con un caso modelo, pero "
            "el CODCLI no coincide exactamente con el alumno "
            "del reporte. Se requiere validar el plan antes "
            "de convertir el nivel."
        )
        ...
        resultado.at[
            ...
            "La carrera se relaciona con más de una estructura "
            "de periodización. No corresponde convertir."
        )
        ...
        resultado.at[
            ...
            "El CODCLI y las carreras del estudiante no "
            "coinciden con los dos casos modelo disponibles."
        )
...
modelos_avance.to_csv(
    RESULTADOS
    / "04_MODELOS_PERIODIZACION_OBSERVADOS.tsv",
    sep="\t",
    index=False,
)
