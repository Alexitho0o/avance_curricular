...
# ============================================================
# NORMALIZAR TIPOS ANTES DE MATERIALIZAR
# ============================================================
#
# La matriz base fue leída con dtype=str. Antes de insertar
# niveles enteros, ambas columnas deben convertirse a Int64
# nullable para evitar incompatibilidad de tipos.

for columna_numerica in [
    "NIVEL_FINAL",
    "ANIO_CURRICULAR_FINAL",
]:
    if columna_numerica not in resultado.columns:
        resultado[columna_numerica] = pd.Series(
            pd.NA,
            index=resultado.index,
            dtype="Int64",
        )
    else:
        resultado[columna_numerica] = pd.to_numeric(
            resultado[columna_numerica]
            .replace("", pd.NA),
            errors="coerce",
        ).astype("Int64")
