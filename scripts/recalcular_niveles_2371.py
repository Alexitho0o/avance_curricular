...
# ============================================================
# LIMPIEZA CONTROLADA DE COLUMNAS DERIVADAS PREEXISTENTES
# ============================================================
#
# La trazabilidad de entrada puede contener columnas calculadas
# en fases anteriores. Este script debe recalcularlas desde las
# fuentes vigentes y no mezclarlas con columnas nuevas mediante
# sufijos _x/_y.
#
# Solo se eliminan de la copia de trabajo en memoria.
# No se modifica ningún archivo fuente.

columnas_derivadas_recalculables = [
    # Evidencia exacta Hoja1
    "CODPESTUD_HOJA1",
    "NIVEL_MIN_HOJA1",
    "NIVEL_MAX_HOJA1",
    "NIVELES_HOJA1",
    ...
    # Trayectorias por documento
    "N_PLANES_DOCUMENTO",
    "PLANES_DOCUMENTO",
    "NIVEL_MAX_GLOBAL_DOCUMENTO",
    "CODCLI_DOCUMENTO",

    # Resumen físico de malla
    ...
    # Resultado de nivel de ejecuciones anteriores
    "NIVEL_FINAL",
    ...
    "FUENTE_NIVEL_FINAL",
    "OBSERVACION_NIVEL",
    "REGLA_TECNICA_NIVEL_ANIO",
    "NIVEL_ANIO_ES_REGLA_OFICIAL",
    "ESTADO_TRAZABILIDAD_FINAL",
]

columnas_derivadas_encontradas = [
    columna
    for columna in columnas_derivadas_recalculables
    if columna in base.columns
]

if columnas_derivadas_encontradas:
    base = base.drop(
        columns=columnas_derivadas_encontradas
    )

print(
    "Columnas derivadas preexistentes retiradas "
    "de la copia de trabajo: "
    f"{len(columnas_derivadas_encontradas)}"
)

if columnas_derivadas_encontradas:
    print(
        "Detalle: "
        + " | ".join(columnas_derivadas_encontradas)
    )
...
# Renombrar explícitamente la llave del resumen de mallas.
# La base 5809 ya puede contener una columna CODPESTUD_NORM
# proveniente de fases anteriores; evitar colisiones _x/_y.
resumen_malla_merge = resumen_malla.rename(
    columns={
        "CODPESTUD_NORM": "CODPESTUD_MALLA_NORM",
    }
).copy()

if resumen_malla_merge[
    "CODPESTUD_MALLA_NORM"
].duplicated().any():
    duplicados = (
        resumen_malla_merge.loc[
            resumen_malla_merge[
                "CODPESTUD_MALLA_NORM"
    ...
    raise RuntimeError(
        "El resumen de mallas contiene CODPESTUD duplicados: "
        + " | ".join(str(valor) for valor in duplicados)
    )
...
if "CODPESTUD_MALLA_NORM" not in resultado.columns:
    raise RuntimeError(
        "El cruce con resumen_malla no generó "
        "CODPESTUD_MALLA_NORM."
    )

if resultado["CODPESTUD_MALLA_NORM"].isna().any():
    faltantes = (
        resultado.loc[
            resultado[
                "CODPESTUD_MALLA_NORM"
    ...
    raise RuntimeError(
        ...
        + " | ".join(str(valor) for valor in faltantes)
    )

if not resultado[
    "CODPESTUD_RESUELTO"
].eq(
    resultado["CODPESTUD_MALLA_NORM"]
).all():
    raise RuntimeError(
        "El CODPESTUD resuelto no coincide con la llave "
        "del resumen de mallas."
    )
