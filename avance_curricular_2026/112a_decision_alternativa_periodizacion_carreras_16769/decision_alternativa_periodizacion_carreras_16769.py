...
def prepare_canonical_units(h111_detail: pd.DataFrame, oferta_mu: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ["NOMBRE_CARRERA", "DURACION_ESTUDIOS_5810", "ANIOS_PERMITIDOS", "FAMILIA_FUNCIONAL", "TIPO_PLAN_CARRERA"]:
        if col in detail.columns:
            detail = detail.drop(columns=[col])
    meta = oferta_mu[["CODIGO_UNICO", "NOMBRE_CARRERA", "DURACION_ESTUDIOS_5810", "ANIOS_PERMITIDOS", "FAMILIA_FUNCIONAL", "TIPO_PLAN_CARRERA"]].copy()
...
def distribution_from_detail(detail: pd.DataFrame, method_col: str, label: str) -> pd.DataFrame:
    ...
    for code, sub in detail.groupby("CODIGO_UNICO", dropna=False):
        ...
        rows.append(
            {
                "CODIGO_UNICO": code,
                "NOMBRE_CARRERA": row0.get("NOMBRE_CARRERA", ""),
