...
def fase_4(ctx: RunContext, _args: argparse.Namespace) -> PhaseResult:
    ...
    codigo_u = required_column(universe, "CODIGO_UNICO")
    plan_col = first_existing(list(universe.columns), ["PLAN_ESTUDIOS", "PLAN_DE_ESTUDIO", "PLAN_ESTUDIO", "PLAN"])

    rows: list[dict[str, Any]] = []
    diffs: list[dict[str, Any]] = []
    no_eval: list[dict[str, Any]] = []
    by_code = bridge_validation.set_index("CODIGO_UNICO").to_dict("index")

    for _, row in universe.iterrows():
        code = row[codigo_u]
        bridge_row = by_code.get(code, {})
        canonical_plan = bridge_row.get("PLAN_DE_ESTUDIO_CANONICO", "")
        pdf = bridge_row.get("ARCHIVO_PDF", row.get("ARCHIVO_PDF", ""))
        matrix_plan = row[plan_col] if plan_col else ""
        estado_plan = "NO_EVALUABLE"
        motivo = ""
        if plan_col:
            if matrix_plan == canonical_plan:
                estado_plan = "PLAN_MATRIZ_COINCIDE_CANONICO"
            else:
                estado_plan = "PLAN_MATRIZ_DIFIERE_CANONICO"
                motivo = "PLAN_MATRIZ_DIFIERE_CANONICO"
                diffs.append(
                    {
                        "CODIGO_UNICO": code,
                        "ARCHIVO_PDF": pdf,
                        "PLAN_MATRIZ": matrix_plan,
                        "PLAN_DE_ESTUDIO_CANONICO": canonical_plan,
                    }
                )
        else:
            motivo = "VALIDACION_PARCIAL_SIN_PLAN_EN_MATRIZ"
            no_eval.append({"CODIGO_UNICO": code, "ARCHIVO_PDF": pdf})
        rows.append(
            {
                "CODIGO_UNICO": code,
                "ARCHIVO_PDF": pdf,
                "COLUMNA_PLAN_MATRIZ": plan_col or "",
                "PLAN_MATRIZ": matrix_plan,
                "PLAN_DE_ESTUDIO_CANONICO": canonical_plan,
                "ESTADO_VALIDACION_PLAN_MATRIZ": estado_plan,
                "MOTIVO_PLAN_MATRIZ": motivo,
            }
        )

    out_validation = output_path(ctx, "02_INTERMEDIOS/04_VALIDACION_PLAN_MATRIZ.tsv")
    out_diffs = output_path(ctx, "04_AUDITORIA/10_PLAN_MATRIZ_DIFIERE_CANONICO.tsv")
    out_no_eval = output_path(ctx, "04_AUDITORIA/11_PLAN_NO_EVALUABLE.tsv")
    salidas = [
        write_tsv(pd.DataFrame(rows), out_validation),
        write_tsv(pd.DataFrame(diffs), out_diffs),
    ...
    return PhaseResult(
        ...
        conteos={
            "carreras_procesadas": len(rows),
            "plan_matriz_difiere_canonico": len(diffs),
            "plan_no_evaluable": len(no_eval),
            "columna_plan_detectada": plan_col or "",
        },
        validaciones=[
            {"validacion": "deteccion_plan_matriz", "columna": plan_col or "", "estado": "OK" if plan_col else "NO_EVALUABLE"},
            {"validacion": "igualdad_exacta_plan", "diferencias": len(diffs)},
        ],
        resumen=[
            "Estado: COMPLETADA",
            f"Columna plan detectada: {plan_col or 'NO'}",
            f"Diferencias plan matriz/canonico: {len(diffs)}",
            f"No evaluables: {len(no_eval)}",
            f"Validacion plan matriz: {out_validation.resolve()}",
        ],
