...
def source_inventory(v2: pd.DataFrame, pending: pd.DataFrame) -> pd.DataFrame:
    ...
    pending_docs = set(pending["DOCUMENTO"].map(norm_doc))
    pending_codcli = set(pending["CODCLI"].map(norm_text))
    rows = []
    ...
    for path in ROOT.rglob("*"):
        ...
        try:
            if path.suffix.lower() in {".csv", ".tsv"}:
                ...
                covered = count_covered(df, keys, pending_docs, pending_codcli)
                rows.append(
                    {
                        "ARCHIVO": str(path),
                        "HOJA": "",
                ...
                if not variable:
                    continue
                covered = sum(1 for d in pending_docs if d and d in text) + sum(1 for c in pending_codcli if c and c in text)
                rows.append(
                    {
                        "ARCHIVO": str(path),
                        "HOJA": "",
...
def count_covered(df: pd.DataFrame, keys: List[str], pending_docs: set, pending_codcli: set) -> int:
    covered = set()
    for key in keys:
        vals = df[key].astype(str).map(norm_doc if "DOC" in key.upper() or "RUT" in key.upper() else norm_text)
        for val in vals:
            if val in pending_docs or val in pending_codcli:
                covered.add(val)
            elif len(val) > 1 and val[:-1] in pending_docs:
                covered.add(val[:-1])
    return len(covered)
...
def create_report(
    ...
    lines = [
        ...
        pd.DataFrame([{"ESTADO": k, "REGISTROS": v} for k, v in status_counts.items()]).to_markdown(index=False),
        "",
        "## Pendientes finales",
    ]
    pend_rows = []
    ...
    lines.append(pd.DataFrame(pend_rows).to_markdown(index=False))
    lines += [
        "",
        "## Limitaciones",
