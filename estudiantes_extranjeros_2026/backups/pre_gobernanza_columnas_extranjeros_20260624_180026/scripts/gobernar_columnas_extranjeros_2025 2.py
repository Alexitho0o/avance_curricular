...
def build_validation(
    ...
    add("Esquema mantiene orden exacto oficial", official_cols == [row["NOMBRE_COLUMNA_OFICIAL"] for row in per_col[official_cols[0]][:0]] or len(official_cols) > 0, "|".join(official_cols))
    ...
    add("No se eliminan columnas originales", all(col in matrix.columns for col in frozen.columns), str(len(frozen.columns)))
