...
- `MODALIDAD` final prioriza lookup en oferta academica por `CODIGO_CARRERA_SIES_FINAL`.
- La fila incluida conserva trazabilidad en `MODALIDAD_FUENTE_FINAL`, `MODALIDAD_METODO_FINAL` y `MODALIDAD_AUDIT_STATUS`.
- El resultado final queda coherente con `JOR`.
...
- Inconsistencias `MODALIDAD` vs regla derivada desde `JOR`: `0`.
